...
name: quarkstudio
description: >-
  QuarkStudio（Quafu / BAQIS 量子云 SDK）的可安装双语安全 runbook。用于：检查用户是否已注册、引导注册、安装或检查 SDK、配置 API token、经 opencli 浏览器登录获取 token、把凭证安全保存到用户级或显式 dotenv 位置、检查 backend status、提交基础 OpenQASM 2.0 任务、轮询结果、处理 token 过期。回答 Quafu 操作问题前必须读取对应 reference；不得打印凭证；未经当前明确授权不得提交真实硬件任务。
---

# QuarkStudio / Quafu skill

QuarkStudio 是 BAQIS Quafu-SQC 的 Python SDK。本 skill 是独立公开 skill：不要假设任何私有仓库结构、registry path、本机 secrets 文件惯例或项目专属数据管线。稳定平台身份以 `reference/platform-info.md` 为准；具体操作步骤以 capability map 指向的 reference 文件和 helper 脚本为准。除非用户明确要求且通过安全门，不要抓取 live 页面，也不要提交真实硬件任务。
...
5. **凭证只存到用户批准的本地位置。** 默认写 XDG 用户配置文件 `$XDG_CONFIG_HOME/quarkstudio/credentials.env` 或 `~/.config/quarkstudio/credentials.env`；只有用户明确选择 per-project dotenv 存储时，才写项目本地 `.env.local`。
...
4. 如果有：通过安全/私密输入通道接收，不要复述 token；默认用 `helpers/store_token.py --destination user-env --token-stdin` 保存，除非用户明确选择项目本地 dotenv 存储。
...
- QuarkStudio install/import/status/run/result 基础：Quafu-SQC 公开文档，已汇总在 `reference/platform-info.md`、`reference/install.md`、`reference/basic-usage.md`。
- 浏览器 token 端点：Quafu-SQC 控制台行为，已汇总在 `reference/auth.md`，包括 `GET /api/api-token`、`POST /api/api-token`、`GET /api/api-token-expiration`，都依赖浏览器 credentials。
- 登录必须保持人工：Quafu-SQC 登录流程可能包含反机器人分支；本 skill 不得自动填写密码或验证码。

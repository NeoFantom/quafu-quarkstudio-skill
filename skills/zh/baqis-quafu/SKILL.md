...
5. **不要把真实硬件任务作为副作用提交。** `tmgr.status()` 和本地 QSteed transpile 检查是安全操作；`tmgr.run(task)` 必须获得当前明确授权，并立即保存 task id。
...
| 真实硬件提交与 secret 安全门 | `reference/safety.md` |
...
3. 询问用户是否已有 Quafu API token。如果有，默认用 `helpers/store_token.py --destination user-env --token-stdin` 保存，除非用户明确选择项目 dotenv 存储。
4. 如果需要浏览器获取，先请求许可，用 `opencli` 打开 `https://quafu-sqc.baqis.ac.cn/login`，让用户手动登录，然后运行 `helpers/retrieve_token_opencli.py --session quafu-token --store user-env`。
...
- QuarkStudio 任务要构造明确的 OpenQASM 2.0 task dict，检查 `shots` 是 1024 的整数倍，说明 backend/circuit/options，再询问是否执行 `tmgr.run(task)`。
...
# 真实 QPU 上可能消耗 quota：先问用户，再立刻保存 tid。
tid = tmgr.run(task)
res = tmgr.result(tid)
```

...
基础 QuarkStudio 不需要 QSteed。如果用户选择启用 BAQIS QSteed compile/transpile 支持，读取 `qsteed.md` 并使用：

```bash
python helpers/qsteed_setup.py          # dry run
python helpers/qsteed_setup.py --yes    # 用户同意后
```

本机验证过的 QSteed 路径使用 Python 3.10/3.11、隔离 venv、`pyquafu==0.4.1` 和 `qsteed==0.2.2`。不要把 QSteed 隐式安装进用户项目。
